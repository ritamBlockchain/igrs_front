...
const STEPS: Step[] = [
  { id: 1, title: 'Register Court Order', actor: 'Registrar', allowedRoles: ['Court Registrar', 'Admin'], description: 'Register case and court decision' },
  { id: 2, title: 'Verify (Collector)', actor: 'Collector', allowedRoles: ['Collector', 'Admin'], description: 'Field verification of court order' },
  ...
  { id: 4, title: 'Execute / Reject Order', actor: 'Revenue Officer', allowedRoles: ['Revenue Officer', 'Admin'], description: 'Transfer per court decision or reject' },
];
  ...
  const handleWorkflowStep = async (action: 'verify' | 'approve' | 'finalize' | 'reject', nextStep?: number) => {
    ...
    try {
      ...
      if (action === 'verify') res = await api.verifyTalati(landId, 'COURT ORDER');
        ...
        res = await api.rejectMutation(landId, 'COURT ORDER', rejectionReason);
      }
      ...
      if (nextStep) {
        ...
        setTimeout(() => {
          setLandId(''); setCaseNumber(''); setCourtName(''); setNewOwner(''); setOrderDate(''); setRejectionReason('');
      ...
      {currentStep === 3 && (
        <StepCard step={STEPS[2]} role={role} onSubmit={() => handleWorkflowStep('approve', 4)} loading={submitting} result={result}>
          <div style={{ padding: 20, background: 'var(--warning-bg)', borderRadius: 12, marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--slate-600)' }}>Status: VERIFIED_BY_COLLECTOR · Ready for executive sign-off</div>
          </div>
      ...
      {currentStep === 4 && (
        ...
        >
          <div style={{ padding: 20, background: 'var(--success-bg)', borderRadius: 12, marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--slate-600)' }}>Status: VERIFIED_BY_DISTRICT_MAGISTRATE · Ownership will be transferred per court decision</div>
          </div>
...
function StepCard({ step, role, children, onSubmit, loading, result, customActions, disabled }: {
  ...
  customActions?: React.ReactNode; disabled?: boolean;
}) {
